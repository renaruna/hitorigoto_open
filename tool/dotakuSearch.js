//PL同卓検索
const pldotakuFormElement = document.getElementById('pldotakuForm');
if (pldotakuFormElement) {
    pldotakuFormElement.addEventListener('submit', async (event) => {
...
//KPPL同卓検索
const kppldotakuFormElement = document.getElementById('kppldotakuForm');
if (kppldotakuFormElement) {
    kppldotakuFormElement.addEventListener('submit', async (event) => {
        // 1. ページのリロード（通常のフォーム送信挙動）を防ぐ
        event.preventDefault();
        //エラーを表示させるところ
        const errorElement = document.getElementById('input_error');
